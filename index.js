...
const displayMovements = function(acc, sort = false) {
  ...
  movs.forEach(function(mov, i) {
    ...
    const html = `<div class="movements_row">
      <div class="movements_type_movements_type-${type}">
        ${i + 1} ${type}
      </div>
        <div class="movements_date">${displayDate}</div>
        <div class="movements_value">${formattedMov} EUR</div>
      </div>`
...
const calcDisplaySummary = function (acc) {
  ...
  labelSumInterest = formatCur(interest, acc.locale, acc.currency);
};
...
btnLogin.addEventListener('click', function(e) {
  ...
  if (currentAccount?.pin === +(inputLoginPin.value)) {
    ...
    inputLoginPin.value = '';
    inputLoginPin.blur();
    // OR inputLoginUsername.value = inputLoginPin.value = ""
...
btnSort.addEventListener('click', function(e) {
  ...
  // change the value of sorted dynamically after click
  sorted = !sorted;
});
